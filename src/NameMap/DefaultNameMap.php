...
class DefaultNameMap implements NameMap
{
    ...
    {
        return $this->map[$tag][$name] ?? $this->getNameMappingStrategy($tag)->toPhpType($name);
    }
    ...
    {
        return $this->aliases[$tag][$phpType] ?? $this->getNameMappingStrategy($tag)->fromPhpType($phpType);
    }
